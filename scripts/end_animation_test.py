...
class WindowGame:
    ...
    def show_real_fireworks_effect(self):
        """Display a full-screen firework effect with bursts of rockets."""
        ...
        def create_explosion(x, y):
            """Create an explosion effect at (x, y) with particles that are long visible and affected by gravity."""
            num_particles = 20
            particles = []
            for _ in range(num_particles):
                ...
                # Boost upward momentum; most particles start with negative dy (upward)
                ...
                particle = {
                    ...
                    "dx": dx,
                    "dy": dy,
                    "life": random.randint(30, 60)  # Longer visibility
                }
            ...
            def animate_explosion():
                ...
                for particle in particles:
                    ...
                    # Increase gravity effect so that particles eventually fall down
                    particle["dy"] += 0.5
                    canvas.coords(
                        particle["id"],
                        particle["x"],
        ...
        def launch_burst():
            """Launch a burst of 10 rockets with random delays."""
            for _ in range(10):
                delay = random.randint(0, 500)  # delay in milliseconds
                fireworks_win.after(delay, launch_rocket)

        def schedule_burst():
            """Schedule bursts repeatedly until the effect duration is reached."""
        ...
        # End the fireworks effect after effect_duration plus a short extra delay.
        fireworks_win.after((effect_duration + 1) * 1000, lambda: (fireworks_win.destroy(), self.end_game_popup()))

    def end_game_popup(self):
        """Show the game over popup and prompt for a new game."""
