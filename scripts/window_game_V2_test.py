...
for datei in os.listdir(karten_ordner):
    ...
    if datei.endswith(".png"):
        ...
        #print(name)
        # Prüfen, ob der Name zu unseren Farben und Werten passt
        for farbe in farben:
            for wert in werte:
...
class WindowGame:
    def __init__(self, root, players):
        ...
        rueckseite_bild_tk = ImageTk.PhotoImage(image=Image.fromarray(rueckseite_bild))

        # Lade die Kartenbilder als Tkinter PhotoImage
        karten_bilder_tk = {}
        for name, bild in karten_bilder.items():
            karten_bilder_tk[name] = ImageTk.PhotoImage(image=Image.fromarray(bild))
        ...
        for r in range(len(WINDOW_LAYOUT)):
            for c in range(len(WINDOW_LAYOUT[r])):
                if WINDOW_LAYOUT[r][c]:
                    ...
                    if card_id is not None:
                        if self.face_up[r][c]:
                            # Karte ist aufgedeckt: Setze das passende Kartenbild
                            card_name = card_id_to_label(card_id)
                            self.buttons[r][c].config(image=karten_bilder_tk[card_name], text="", state=tk.DISABLED)
                        else:
                            # Karte ist verdeckt: Setze das Rückseitenbild
